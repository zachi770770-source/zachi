...
const REDIRECTS: Record<string, string> = {
  ...
  "/articles": "/book",
  /**
   * ‎/About-Us.html‎ — כתובת מהאתר הקודם שעמד על הדומיין הזה, והיחידה מתוכו
   * שיש לה עדות ממשית: היא עדיין מופיעה בתוצאות-חיפוש לדומיין, תחת הכותרת
   * „צחי חן מרצה ומנטור להצלחה - אודותיי”, ומאז המעבר היא מחזירה 404.
   *
   * היעד הוא ‎/author‎ ולא דף-הבית: שתי הכתובות הן „מי האדם הזה”, וזו ההתאמה
   * הסמנטית היחידה בין האתר הישן לחדש. **זו הכתובת הישנה היחידה שמופנית כאן.**
   * שאר עמודי האתר הקודם (ייעוץ עסקי, הרצאות, העצמה) אינם עוסקים בזוגיות ואין
   * להם מקבילה — הפניה שלהם לעמודי-תוכן הייתה בדיוק ההפניה-ההמונית שיש להימנע
   * ממנה. מיפוי רחב יותר ימתין לרשימת ה-404 האמיתית מ-Search Console.
   *
   * מופיעה בשתי צורות-אות כי כתובות ישנות מצוטטות בשתיהן, ו-‎matcher‎ של Next
   * משווה תווית-רגישות. אין כאן נרמול גורף של אותיות — שתי כניסות מפורשות.
   */
