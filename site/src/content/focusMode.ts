 ...
 * מספר או תוכן מהספר, ותואם את גבולות התוכן של האתר.
 */
...
const COMPOSITION: Record<HomePathId, { journey: JourneyId; fact: string; story: string }> = {
  ...
  // מתחילים מחדש: דייט שנדחה → „איבד/ה עניין”. אותו זוג מאושר של „לפני קשר”,
  // כי זו אותה חזרה לעולם ההיכרויות — בלי לייצר טקסט חדש.
  restart: { journey: "starting-again", fact: "postponed", story: "כנראה איבד/ה בי עניין" },
};
...
export const focusUi = {
  ...
  /** הכפתור היחיד שמפעיל את ההפרדה (שלב 1 → שלב 2). */
  separateLabel: "הפרידו: מה מזה באמת קרה?",
  /**
   * ה-CTA הסוגר. **חייב להיות קישור אמיתי אל הספר.**
   *
   * זה היה הליקוי החמור באתר: הכפתור הזה לא ניווט לשום מקום — הוא החליף את
   * המקטע בשאלון נוסף, שהשאלה השנייה בו היא „האם יש בבית פחד, שליטה, פגיעה או
   * סכנה?”. מבקר שלחץ „המשיכו עם הספר” נשאל על אלימות בבית. עכשיו הכפתור הוא
   * `<Link href="/book">` ותו לא.
   */
  continueLabel: "המשיכו עם הספר",
  /** יציאה מהתרגיל. */
  backLabel: "סגירה",
  /**
   * מסגור-האמת של הדוגמה. קודם השלב הראשון ביקש „קחו רגע אחד שקרה לכם
   * לאחרונה”, ומיד אחר-כך הוצגה הדוגמה *של האתר*. המבקר התבקש להתבונן פנימה
   * ואז קיבל דוגמה של מישהו אחר. עכשיו נאמר במפורש שזו דוגמה מהספר.
   */
  exampleNote: "דוגמה מתוך הספר.",
} as const;
