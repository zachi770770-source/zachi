  ...
  return (
    <DialogPrimitive.Root open={open} onOpenChange={setOpen}>
      ...
      <DialogPrimitive.Portal>
        {/* כניסה *ויציאה* בשני הכיוונים. Radix Presence משהה את הפירוק עד תום
            אנימציית ה-`closed`, ולכן די בכך; תחת prefers-reduced-motion התנועה
            מבוטלת גלובלית, animationName הופך ל-none, והסגירה חוזרת להיות
            מיידית — בלי טיימר ובלי התנהגות מיוחדת. */}
        <DialogPrimitive.Overlay className="fixed inset-0 z-50 bg-foreground/50 backdrop-blur-[2px] data-[state=closed]:animate-fade-out data-[state=open]:animate-fade-in lg:hidden" />
        <DialogPrimitive.Content
          className="fixed inset-y-0 start-0 z-50 flex w-[min(85vw,22rem)] flex-col gap-6 overscroll-contain bg-surface p-6 shadow-lg focus:outline-none data-[state=closed]:animate-drawer-out data-[state=open]:animate-drawer-in lg:hidden"
