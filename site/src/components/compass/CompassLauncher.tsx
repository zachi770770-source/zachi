...
export function CompassLauncher() {
  ...
  /**
   * ‎/preview‎ במובייל — החרגה שנכפתה על-ידי התנגשות אמיתית, ולא העדפה שלי.
   *
   * ‎e2e/preview.spec.ts‎ שומר במפורש שבעמוד-הטעימה במובייל לא תרחף *שום*
   * בקרה מלבד ההדר וסרגל-הקורא, בכל מיקום-גלילה. ההערה שם מתעדת שזו החלטה
   * מדודה — „עמוד-הטעימה הוא חוויית קריאה שקטה וכל שכבת-שיווק שמרחפת מעליה
   * מתחרה בה” — ושניסוח רחב מדי כבר צומצם שם פעם אחת בכוונה.
   *
   * הבקשה לשחזר את המשגר מנתה את ‎/preview‎ ברשימת העמודים. שתי הדרישות
   * אינן יכולות להתקיים יחד במובייל, ולכן נבחרה ההחרגה הצרה ביותר: המשגר
   * נעלם ב-‎/preview‎ *במובייל בלבד*, ונשאר שם בדסקטופ ובטאבלט. לא נגעתי
   * בבדיקה: היא מבטאת החלטת-מוצר, והחלשתה כדי „לעבור” הייתה מוחקת אותה.
   */
  if (pathname?.startsWith("/preview") && isMobile) return null;
