 ...
 * כשהוא נוחת. המעטפת הזו נפתחת לאורך 22% הראשונים, מחזיקה לאורך החצייה,
 * ונסגרת לאורך 20% האחרונים. היא מה שהופך „מישור שמסתובב” ל„גיליון שנתלש
 * מערימה, חוצה, ונבלע בערימה השנייה”.
 */
const liftEnvelope = (t: number) => {
  const x = clamp01(t);
  const open = smooth(clamp01(x / 0.22));
  const close = 1 - smooth(clamp01((x - 0.8) / 0.2));
  return open * close;
};
