...
/**
 * דף שמתהפך — **התקדמות זוויתית** לאורך הדפדוף.
 *
 * הגרסה הקודמת („הרמה מהירה, הנחה איטית”) בלעה את שני הקצוות: ב-18% מהזמן
 * הגיליון כבר היה מורם ושטוח מעל הדף שמתחתיו — כלומר ההיפרדות מהערימה לא
 * נראתה מעולם — וב-85% הוא כבר שכב. מה שנראה בפועל היה קשת אמצעית בלבד.
 *
 * quintic smootherstep נותן בדיוק את מאזן-הפאזות שנדרש כאן, והוא רציף בנגזרת
 * *ובתאוצה* בשני הקצוות, ולכן אין „ניתוק” בהתחלה ואין נחיתת-סנאפ בסוף:
 *
 *   22% הראשונים  →  ‎~7%‎ מהסיבוב   (הרמה: הגיליון נפרד, כמעט לא מסתובב)
 *   58% האמצעיים  →  ‎~87%‎ מהסיבוב  (החצייה)
 *   20% האחרונים  →  ‎~6%‎ מהסיבוב   (נחיתה: משתטח ונדחס אל הערימה)
 */
...
/**
 * מעטפת-ההיפרדות: כמה הגיליון מורם *מעל* גוש-הדפים, בנפרד מהזווית שלו.
 *
 * זה התיקון המרכזי. עד כה גם ההרמה בציר-z וגם צל-המגע נגזרו מ-`sin(θ)`, שהוא
 * אפס בדיוק בשני הקצוות — ולכן הצל לא נפתח כשהדף עוזב את הערימה ולא נסגר
 * כשהוא נוחת. המעטפת נפתחת לאורך 22% הראשונים ונסגרת לקראת הסוף, והיא מה
 * שהופך „מישור שמסתובב” ל„גיליון שנתלש מערימה, חוצה, ונבלע בערימה השנייה”.
 *
 * ── למה הנחיתה חייבת לחיות *כאן* ולא בזווית ──────────────────────────────
 * ליד ‎θ=180°‎ הסיבוב אסימפטוטי: בין 75% ל-95% מהזמן הזווית זזה מ-‎~169°‎
 * ל-‎~178°‎, וזה הפרש שאי-אפשר לראות. לכן „התקרבות → השתטחות → היבלעות” אינה
 * יכולה להימסר על-ידי `pageTurn`, ומי שמוסר אותה היא המעטפת: הגובה מעל
 * גוש-הדפים, וצל-המגע שנגזר ממנו.
 *
 * הסגירה הקודמת (smoothstep על 20% האחרונים) התחילה מאוחר מדי ונגמרה מהר
 * מדי: ב-85% הגיליון כבר היה מונח, ולכן 75/85/95 נראו כמעט זהים. עכשיו
 * הירידה מתחילה ב-60% ומתפרשת על 40% מהזמן, בעקומה קוּבּית שמחזיקה גובה
 * ואז צונחת — ‎0.98 · 0.88 · 0.58 · 0.33 · 0‎ ב-70/80/90/95/100. שלושת הרגעים
 * (חצייה · התקרבות · היבלעות) נעשים נפרדים לעין, בלי להאט את הדפדוף כולו
 * ובלי ריבאונד: העקומה מונוטונית יורדת ואינה חוצה אפס.
 */
const liftEnvelope = (t: number) => {
  const x = clamp01(t);
  const open = smooth(clamp01(x / 0.22));
  const u = clamp01((x - 0.6) / 0.4);
  const close = 1 - u * u * u;
  return open * close;
};

/**
 * הדחיסה אל תוך גוש-הדפים — 12% האחרונים בלבד.
 *
 * ברגע שגיליון נוגע בערימה הוא נלחץ אליה ומשתטח מעט מעבר לעיקול-המנוחה שלו.
 * זה הפרט שגורם ל„הערימה קיבלה דף” במקום „הדף נעשה שטוח”. מונוטוני ועדין
 * (‎0.1‎ רדיאן), ומיד אחריו זנב-ההתיישבות הקיים מחזיר אותו — כלומר לחיצה
 * ושחרור, לא קפיצה.
 */
const landPress = (t: number) => smooth(clamp01((clamp01(t) - 0.88) / 0.12));
...
export function createBookScene(
  ...
  // מובייל 4 → 6: בארבעה גיליונות לצד גוש-הדפים במובייל נקרא כשתי פרוסות,
  // ועובי-הספר הוא דרישה שווה בשני המשטחים. שישה עדיין זולים משבעה.
  const FAN_PER_SIDE = opts.mobile ? 6 : 7;
  ...
  // 0.1 → 0.17: ההרמה חייבת להיות גדולה מספיק כדי שהגיליון ייקרא כ*מעל*
  // הערימה ולא כמונח עליה. בערך הישן, בשלב-ההרמה הוא היה כמעט משיק לדף
  // שמתחתיו — ומכיוון ששניהם נושאים תוכן, זה נראה כטקסט כפול ולא כהיפרדות.
  const Z_LIFT = 0.17;
  /**
   * שארית-ההושבה: כמה הגיליון נשאר מעל הדף שמתחתיו ברגע שהוא כבר שטוח.
   * 0.012 — פחות מעשירית מ-`Z_LIFT`, ולכן אינו נראה כריחוף; די בו כדי
   * שהעין תקרא שני משטחים ולא משטח אחד עם טקסט כפול.
   */
  const SEAT_Z = 0.012;

  // עיקול-מנוחה תלוי-צד: ימין +, שמאל − (כדי ששני העמודים יתרוממו כלפי חוץ).
  // צד-שמאל נושא מקדם ‎1.09‎ — הערימה שם מעט יותר עמוקה.
  const REST_CURL_SRC = -REST_CURL * 1.09; // ערימת-המקור (שמאל)
  const REST_CURL_DST = REST_CURL; // ערימת-היעד (ימין)
  ...
  const turnDur: number[] = [];
  {
    ...
    for (let n = 0; n < 1400; n += 1) {
      turnStart.push(acc);
      // 1.42–1.62 שניות. קודם: 0.78–0.98 — מהיר מכדי לעקוב אחרי ההיפרדות,
      // העיקול והנחיתה. הטווח נשאר *טווח* ולא ערך אחד, כדי שלא יהיה מטרונום.
      turnDur.push(1.42 + 0.2 * rnd(n + 3));
      const cycleEnd = (n + 1) % TURNS_PER_CYCLE === 0;
      // המרווח בין התחלות חייב להיות גדול מהמשך, אחרת דף מתחיל לפני שקודמו
      // נחת. 1.70–2.04 מול משך של עד 1.62 ⇒ 0.08–0.62 שניות של שקט בין דף
      // לדף. הפוגת-הקריאה בסוף מחזור גדלה בהתאמה.
      acc += 1.7 + 0.34 * rnd(n) + (cycleEnd ? 3.2 + 1.0 * rnd(n + 11) : 0);
    }
  }
  ...
  function setAmbient(idleSec: number, scrollP: number) {
    ...
    // ‎×0.86‎: הערימה השמאלית מאמצת את הכפולה הנכנסת מעט *לפני* שהגיליון יושב
    // סופית. זו אותה טכניקה שכבר נהוגה כאן לשכבות קבורות, מאותה סיבה בדיוק:
    // העיקול מקצר את הגיליון, ולכן בשוליים שלו מציץ הדף שמתחתיו. כל עוד
    // התוכן שונה, ההצצה הזו נקראת כשני טקסטים על אותו מישור — וזה מה שנראה
    ...
    for (let j = 0; j < POOL; j += 1) {
      ...
      if (t >= tS) {
        ...
        // ההיפרדות מהערימה — מעטפת נפרדת מהזווית (ראו `liftEnvelope`).
        const sep = liftEnvelope(prog);
        ...
        // עיקול-המנוחה נוסע *עם* הגיליון: מעיקול-המנוחה של ערימת-המקור (שמאל,
        // שלילי) אל זה של ערימת-היעד (ימין, חיובי).
        //
        // כאן ישב הליקוי שגרם להסתרת תחילת-השורות: הביטוי היה
        // ‎restCurlAt(1 - ta)‎, כלומר הוא התחיל ב-‎+REST_CURL‎ וסיים ב-‎-REST_CURL‎.
        // זו הייתה השארית האחרונה של הכיוון הישן (ימין → שמאל): אחרי היפוך
        // הכיוון ב-‎f3f6540‎ הזווית התהפכה אבל האינטרפולציה של העיקול לא.
        // התוצאה נמדדה: גיליון שזה עתה נחת על הערימה הימנית ישב בעיקול
        // ‎-0.344‎, בעוד שכל שכניו על אותה ערימה — דף-הבסיס ‎(+0.228)‎ והגיליונות
        // הקבורים ‎(+0.176, +0.232)‎ — מתעקלים לכיוון ההפוך. כלומר הוא התכופף
        // *לתוך* הערימה במקום להתרומם ממנה, ולכן הרצועה שליד הקצה החופשי
        // צללה מתחת לשכבות שמתחתיה והטקסט שם פשוט נעלם. זה גם הסביר למה
        // הליקוי נראה רק במנוחה: באמצע הדפדוף הגיליון מורם, ואין מה שיסתיר.
        //
        // דרך-אגב תוקנה כאן גם קפיצה שלא נתפסה קודם: ב-‎ta=0‎ הביטוי הישן נתן
        // ‎+0.4‎ בעוד שהגיליון הממתין יושב על ‎-0.436‎ — כלומר קפיצת-עיקול של
        // ‎0.84‎ בפריים הראשון של כל דפדוף. עכשיו שני הקצוות מתלכדים בדיוק.
        curl =
          lerp(REST_CURL_SRC, REST_CURL_DST, ta) -
        ...
        // הראשונים (ולא מתגלה רק באמצע), ונדחס אליה בחזרה ב-20% האחרונים.
        // הפרדת-הושבה: שארית זעירה שמחזיקה את הגיליון *מעל* הדף שמתחתיו עד
        // שהוא באמת יושב. בלעדיה, ב-90–100% המעטפת כבר קרובה לאפס והשניים
        // נעשים כמעט קו-מישוריים — ומכיוון שהעיקול מקצר את הגיליון מעט, הדף
        // שמתחת מציץ בשוליים, ושני הטקסטים נקראים כמודפסים על אותו מישור.
        // הדעיכה תלויה ב-`after` (הזמן מאז הנחיתה) ולא ב-`prog`, ולכן היא
        // ממשיכה לרדת אחרי סוף הדפדוף: רציפה, מונוטונית, בלי קפיצה ובלי
        // ריבאונד — זנב-ההתיישבות הקיים הוא שמושיב אותה סופית תוך ~0.3ש.
        z = lerp(zRight(0), zLeft(1), ta) + sep * Z_LIFT + SEAT_Z * Math.exp(-9 * after);
        depth = 0;
