...
export function MotionRoot() {
  useIsomorphicLayoutEffect(() => {
    ...
    /**
     * „כבר בתצוגה בטעינה” — ולכן נחשף מיד, בלי אנימציה שאיש לא יראה.
     *
     * הסף הוא 85% מגובה-החלון ולא 100% בכוונה. נמדד: בדסקטופ ביט-הזיהוי נחשף
     * ב-`scrollY=0` כשראשו נמצא 36px *מתחת* לקיפול — כלומר הכוריאוגרפיה שלו
     * (חוט → כותרת → ציטוט) רצה כשהוא מחוץ למסך, והמבקר גלל אל תוצאה סטטית.
     * הסיבה: הסריקה הראשונה רצה לפני שהפריסה התייצבה (גופנים, רצועת-השפה),
     * וברגע ההוא ראשו כן היה מעל הקיפול. שוליים של 15% סופגים בדיוק את ההפרש
     * הזה: מה שממש בתצוגה נחשף מיד, ומה שיושב על הגבול ממתין לגלילה.
     */
