      ...
      try {
        ...
        ctx = gsap.context(() => {
          ...
          mm.add("(min-width: 1px)", () => {
            ...
            // היא בנייה” נבנית אות-אחר-אות. הטריגר של האותיות (0.50) בא *מיד אחרי*
            // שהחיפוש דעך (מסתיים ~0.48) — כך שאין „מסך ירוק ריק” מורגש, אך הסדר
            // לעולם אינו מתערבב (החיפוש נעלם לפני שהאות הראשונה מופיעה).
            tl
              ...
              // עד ~0.48 — קריא 0→0.26, ואז דעיכה מהירה יותר, כך שהבנייה מתחילה
              // כמעט מיד אחריו (חיתוך ~75% של „הזמן הריק”).
              .to(q(".s2b__search"), { autoAlpha: 0, y: -34, scale: 0.92, duration: 0.22 }, 0.26)
              // (4) הקו נמשך בהדגשה (יסוד המבנה לפני הצמתים)
              .to(q(".s2b__line"), { strokeDashoffset: 0, duration: 0.3 }, 0.38)
              // הצמתים מתמצקים — עלייה חלקה ויציבה (ללא פעימת-טבעת מהבהבת)
              .to(q(".s2b__node"), { autoAlpha: 1, scale: 1, stagger: 0.05, duration: 0.18, ease: "power2.out" }, 0.46)
              // (5) „אהבה היא בנייה” נבנית אות-אחר-אות — הטריגר מפעיל חשיפת-CSS
              // מבוססת-זמן (stagger בין גרפמות), והמשפט המורכב מוחזק לקריאה.
              .call(revealBuild, undefined, 0.5)
