...
/**
 * ארבעת עמודי-המסע = ארבע חוויות נפרדות. כל עמוד: H1 ייחודי, intro, שלושה
 * נושאים, טעימה מותאמת (Primary → /preview?tool=&station=), „שאל את הספר”
 * (משני, עם הקשר-המצב), „בחרו מסלול אחר”, ו-*אינו* מציג מחדש את בורר ארבעת
 * המצבים. „אחרי פרידה” בלבד מציע בעדינות „מתחילים מחדש”.
 */
...
const JOURNEYS = [
  ...
  },
  {
    ...
    topic: "מה נמצא מתחת לריב החוזר",
    tool: "twenty-maintenance",
    station: "inside-relationship",
    ask: "existing",
    others: ["/before-relationship", "/building-relationship", "/after-breakup"],
  },
  {
    path: "/after-breakup",
    h1: "לא צריך לדעת כבר עכשיו מה הצעד הבא.",
    introBit: "פרידה לא מסתיימת ברגע שהקשר נגמר",
    topic: "געגוע לאדם מול געגוע למה שהקשר הבטיח",
    tool: "quiet-check",
...
for (const j of JOURNEYS) {
  test(`${j.path}: unique hero + topics + contextual sample + ask context + no menu re-shown`, async ({
    ...
    // Primary: טעימה מותאמת → הכלי+התחנה הנכונים.
    await expect(
      cta.getByRole("link", { name: /קראו טעימה שמתאימה/ }),
...
test("after-breakup gently offers 'starting again' as a next step (not a primary CTA)", async ({ page }) => {
  ...
  await expect(page.getByText(/מרגישים שכבר מתחיל להיפתח מקום לחדש/)).toBeVisible();
  await expect(page.getByRole("link", { name: "מתחילים מחדש" })).toHaveAttribute(
    "href",
    "/starting-again",
  );
  // הפעולה הראשית נשארת הטעימה — לא „מתחילים מחדש”.
  await expect(page.getByRole("link", { name: /קראו טעימה שמתאימה/ })).toBeVisible();
});
