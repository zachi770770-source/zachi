...
| `SUPABASE_SERVICE_ROLE_KEY` | **צד שרת בלבד.** משמש למחיקת משתמש מ-Supabase Auth דרך `/api/account/delete`. ⚠️ בלי קידומת `NEXT_PUBLIC` ולעולם לא בצד לקוח. אם חסר — מחיקת חשבון מוחקת את כל נתוני המשתמש, אך לא את רשומת ה-Auth. |

### עמודי אמון ובטיחות
`/privacy` (מדיניות פרטיות), `/terms` (תנאי שימוש) ו-`/help` (קבלת עזרה מקצועית ובטיחותית) — ציבוריים, נגישים ללא התחברות, ומקושרים מהפוטר ומההגדרות.

> ⚠️ **לפני השקה ציבורית:** בעמוד `/help` מופיעים רק מספרי החירום הכלליים (משטרה 100, מד״א 101). יש להשלים ידנית מספרי מוקדי סיוע ייעודיים (אלימות במשפחה, מצוקה רגשית) — ראו את ה-TODO בקובץ `src/app/help/page.tsx`. אין להמציא מספרים.
